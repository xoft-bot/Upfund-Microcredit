...
interface AuditQuery { after?: string; limit?: number; correlationId?: string; branchId?: string; }
...
export function registerTelemetryRoutes(app: FastifyInstance, verifier?: TokenVerifier, resolveUser?: UserResolver): void {
  ...
  app.get<{ Querystring: AuditQuery }>('/api/v1/telemetry/audit-stream', {
    preHandler: [
      ...protectedTelemetry,
      requireBranchScope((request) => (request.query as AuditQuery | undefined)?.branchId ?? request.actor?.branchId ?? undefined),
    ],
    schema: {
      querystring: {
        type: 'object',
    ...
    const branchId = actor.role === 'admin' ? (request.query.branchId ?? null) : (actor.branchId ?? null);
    return { ok: true, data: await streamAuditEvents({ ...request.query, branchId }), correlationId: request.headers['x-correlation-id'], version: SYSTEM_VERSION };
  });
}