...
import { assessApplicationRisk, createClient, createLoanApplication, decideApplication, disburseLoan, getApplicationTimeline, getLoanSchedule, getPortalOverview, reviewKyc, submitLoanApplication, transitionLoan } from '../services/lifecycle.js';
...
interface KycBody { status: 'verified' | 'rejected'; verificationMethod: string; evidenceNotes: string; }
interface RiskBody { score: number; riskGrade: string; status: 'approved' | 'declined'; policyVersion: string; rationale: string; }
...
export function registerLifecycleRoutes(app: FastifyInstance, verifier?: TokenVerifier, resolveUser?: UserResolver): void {
  ...
  app.get<{ Params: { id: string } }>('/api/v1/loan-applications/:id/timeline', {
    ...
    schema: { params: { type: 'object', required: ['id'], properties: { id: { type: 'string', minLength: 1 } } } },
  }, async (request) => ({ ok: true, data: await getApplicationTimeline(request.actor!, request.params.id), correlationId: request.headers['x-correlation-id'], version: SYSTEM_VERSION }));
