...
/**
 * Single source of truth for row visibility on every read endpoint.
 * 1) Branch predicate (explicit branch_id, defense in depth beside RLS). Admin without ?branchId sees all.
 * 2) Client role: only its own records. 3) Officer: only records that trace to an application it created (created_by).
 * 4) Collector: only loans with an active collector assignment. Anything not listed is denied for that role.
 */
export function scopeFor(actor: Actor, kind: ScopeKind, alias: string, requested?: string, start = 1): Scope {
  const values: string[] = []; const parts: string[] = []; let n = start;
  const bind = (v: string): string => { values.push(v); return `$${n++}`; };
  if (actor.role === 'client') {
    if (!actor.clientId) return denyAll(start);
  ...
  if (actor.role === 'admin' && !requested) parts.push('TRUE');
  else {
    const branch = requested ?? actor.branchId;
    if (!branch || (actor.role !== 'admin' && branch !== actor.branchId)) return denyAll(start);
    parts.push(`${alias}.branch_id = ${bind(branch)}`);
  }
  if (actor.role === 'officer') {
    const me = (): string => bind(actor.dbUserId);
    if (kind === 'application') parts.push(`${alias}.created_by = ${me()}`);
...
function likeOf(term: string): string { return `%${term.replace(/[\\%_]/g, (ch) => `\\${ch}`)}%`; }
...
async function pagedApplications(actor: Actor, query: PageQuery) {
  ...
  const scope = scopeFor(actor, 'application', 'la', query.branchId);
  const values = [...scope.values]; let n = scope.next; const where = [scope.sql, applicationQueueSql(query.queue)];
  if (query.q) { where.push(`(c.display_name ILIKE $${n} OR c.external_ref ILIKE $${n} OR la.id::text ILIKE $${n})`); values.push(likeOf(query.q)); n++; }
...
async function pagedLoans(actor: Actor, query: PageQuery) {
  ...
  const scope = scopeFor(actor, 'loan', 'l', query.branchId); const values = [...scope.values]; let n = scope.next; const where = [scope.sql];
  ...
  if (query.q) { where.push(`(c.display_name ILIKE $${n} OR c.external_ref ILIKE $${n} OR l.id::text ILIKE $${n})`); values.push(likeOf(query.q)); n++; }
  const predicate = where.join(' AND ');
...
async function pagedClients(actor: Actor, query: PageQuery) {
  const page = pageOf(query); const scope = scopeFor(actor, 'client', 'c', query.branchId); const values = [...scope.values]; let n = scope.next; const where = [scope.sql];
  if (query.q) { where.push(`(c.display_name ILIKE $${n} OR c.external_ref ILIKE $${n})`); values.push(likeOf(query.q)); n++; }
...
async function pagedPayments(actor: Actor, query: PageQuery) {
  ...
  const scope = scopeFor(actor, 'payment', 'p', query.branchId); const values = [...scope.values]; let n = scope.next; const where = [scope.sql];
  if (status) { where.push(`p.status = $${n}`); values.push(status); n++; }
  if (query.q) { where.push(`(p.receipt_reference ILIKE $${n} OR p.id::text ILIKE $${n} OR c.display_name ILIKE $${n})`); values.push(likeOf(query.q)); n++; }
...
async function singleClient(actor: Actor, id: string) {
  const scope = scopeFor(actor, 'client', 'c', undefined, 2); const result = await pool.query(`SELECT c.id, c.external_ref AS "externalRef", c.display_name AS "displayName", c.branch_id AS "branchId", c.created_at AS "createdAt" FROM clients c WHERE c.id = $1 AND ${scope.sql}`, [id, ...scope.values]);
...
export function registerReadApiRoutes(app: FastifyInstance, verifier?: TokenVerifier, resolver?: UserResolver): void {
  ...
  app.get('/api/v1/search', { preHandler: [auth, requireRoles(searchRoles), requireBranchScope(requestedBranch)], schema: querySchema() }, async (request) => {
    ...
    const run = (kind: ScopeKind, alias: string, build: (predicate: string, likeParam: string) => string) => {
      const scope = scopeFor(actor, kind, alias, query.branchId); const likeParam = `$${scope.next}`;
      return pool.query(build(scope.sql, likeParam), [...scope.values, like]);
    };
    const accountantOnly = actor.role === 'accountant'; // accountants read payment evidence, not client/application/loan records
    const none = Promise.resolve({ rows: [] as unknown[] });
    const [clients, loans, applications, receipts] = await Promise.all([
      accountantOnly ? none : run('client', 'c', (w, k) => `SELECT c.id, c.external_ref AS "externalRef", c.display_name AS "displayName", c.branch_id AS "branchId" FROM clients c WHERE ${w} AND (c.display_name ILIKE ${k} OR c.external_ref ILIKE ${k}) ORDER BY c.display_name, c.id LIMIT 20`),
      accountantOnly ? none : run('loan', 'l', (w, k) => `SELECT l.id, l.client_id AS "clientId", c.display_name AS "clientName", l.branch_id AS "branchId", l.status FROM loans l JOIN clients c ON c.id = l.client_id WHERE ${w} AND (l.id::text ILIKE ${k} OR c.display_name ILIKE ${k} OR c.external_ref ILIKE ${k}) ORDER BY l.created_at DESC, l.id DESC LIMIT 20`),
      accountantOnly ? none : run('application', 'la', (w, k) => `SELECT la.id, la.client_id AS "clientId", c.display_name AS "clientName", la.branch_id AS "branchId", la.status FROM loan_applications la JOIN clients c ON c.id = la.client_id WHERE ${w} AND (la.id::text ILIKE ${k} OR c.display_name ILIKE ${k} OR c.external_ref ILIKE ${k}) ORDER BY la.created_at DESC, la.id DESC LIMIT 20`),
      run('payment', 'p', (w, k) => `SELECT p.id, p.receipt_reference AS "receiptReference", p.loan_id AS "loanId", p.branch_id AS "branchId", p.status FROM payments p WHERE ${w} AND p.receipt_reference ILIKE ${k} ORDER BY p.created_at DESC, p.id DESC LIMIT 20`),
    ]);
  ...
  app.get('/api/v1/audit', { preHandler: [auth, requireRoles(auditRoles), requireBranchScope(requestedBranch)], schema: querySchema({ entityType: { type: 'string', minLength: 1, maxLength: 100 }, entityId: { type: 'string', minLength: 1 }, q: { type: 'string', maxLength: 100 } }) }, async (request) => {
    const query = request.query as PageQuery & { entityType?: string; entityId?: string }; const page = pageOf(query); const scope = scopeFor(request.actor!, 'audit', 'ae', query.branchId); const values = [...scope.values]; let n = scope.next; const where = [scope.sql]; if (request.actor!.role !== 'admin') where.push('ae.branch_id IS NOT NULL');
