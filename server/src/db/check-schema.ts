...
const required = ['users', 'roles', 'permissions', 'branches', 'clients', 'loan_products', 'loan_applications', 'loans', 'repayment_schedules', 'payments', 'receipts', 'ledger_transactions', 'ledger_entries', 'capital_pools', 'audit_events', 'businesses', 'kyc_records', 'risk_assessments', 'reconciliations', 'reconciliation_payments', 'allocation_policies', 'pool_allocations'];
...
console.log(`Verified ${required.length} Stage 1/2 tables`);
