...
export interface ReconciliationCronOptions { actorUserId: string; policyVersion: string; asOf?: Date; varianceAlertThreshold?: number; dryRun?: boolean; }
export interface ReconciliationCronDependencies { loadCandidates?: (asOf: Date) => Promise<Candidate[] | null>; expectedForBranch?: (branchId: string, asOf: Date) => Promise<number>; postBatch?: typeof postReconciliationBatch; quarantine?: (input: BatchInput & { correlationId: string }) => Promise<void>; alertSink?: VarianceAlertSink; acquireLock?: <T>(work: () => Promise<T>) => Promise<T | null>; }
...
export async function runReconciliationCycle(options: ReconciliationCronOptions, dependencies: ReconciliationCronDependencies = {}): Promise<ReconciliationCronResult> {
  const run = async (): Promise<ReconciliationCronResult> => {
    const asOf = options.asOf ?? new Date();
    const candidates = await (dependencies.loadCandidates ?? loadCandidates)(asOf);
    if (!candidates) return { processed: 0, posted: 0, quarantined: 0, skipped: true };
    ...
    for (const batch of batches) {
      ...
      if (result.status === 'matched') { if (options.dryRun) { posted += 1; continue; } await (dependencies.postBatch ?? postReconciliationBatch)({ actorUserId: options.actorUserId, actorRole: 'admin', branchId: batch.branchId, batchReference: reference, expectedAmount: result.expectedAmount, recordedAmount: result.recordedAmount, submittedAmount: result.submittedAmount, paymentIds: batch.paymentIds, policyVersion: options.policyVersion, managerOverride: false, correlationId }); posted += 1; continue; }
      const quarantineInput = { ...batch, batchReference: reference, correlationId, expectedAmount: result.expectedAmount, recordedAmount: result.recordedAmount, submittedAmount: result.submittedAmount, variance: result.variance }; if (!options.dryRun && dependencies.quarantine) await dependencies.quarantine(quarantineInput); else if (!options.dryRun) await withTransaction(async (client) => { await quarantineVarianceBatchOnClient(client, { actorUserId: options.actorUserId, branchId: batch.branchId, batchReference: reference, expectedAmount: result.expectedAmount, recordedAmount: result.recordedAmount, submittedAmount: result.submittedAmount, variance: result.variance, paymentIds: batch.paymentIds, correlationId }); });
  ...
  const acquireLock = dependencies.acquireLock ?? (dependencies.loadCandidates
    ? ((work: () => Promise<ReconciliationCronResult>) => work())
    : ((work: () => Promise<ReconciliationCronResult>) => withAdvisoryLock('letsgrow:reconciliation-cron', work)));
  const result = await acquireLock(run);
  return result ?? { processed: 0, posted: 0, quarantined: 0, skipped: true };
}
