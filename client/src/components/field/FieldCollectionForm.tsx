...
interface FieldCollectionFormProps { queue: OfflineQueue; collectorId: string; branchId: string; deviceId: string; onQueued: (record: FieldCollectionRecord) => void; disabled?: boolean; }
const makeId = (): string => globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;

export function FieldCollectionForm({ queue, collectorId, branchId, deviceId, onQueued, disabled = false }: FieldCollectionFormProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (disabled) return; setError(''); setSaved(false); const parsedAmount = Number(amount);
