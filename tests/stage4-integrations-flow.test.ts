...
describe('Stage 4 integration flow', () => {
  ...
  it('protects telemetry endpoints and masks secrets in audit output', async () => {
    const query = vi.spyOn(pool, 'query').mockImplementation(async (sql: string) => { if (sql === 'SELECT 1') return { rows: [{ '?column?': 1 }], rowCount: 1 } as never; if (sql.includes('COUNT(*)')) return { rows: [{ pending_payments: '2', pending_field_collections: '1', variance_batches: '3' }], rowCount: 1 } as never; return { rows: [{ id: 'audit-1', action: 'login', entity_type: 'user', entity_id: 'user-1', correlation_id: 'corr-1', metadata: { token: 'secret', national_id: 'N-1', safe: 'ok' }, created_at: new Date('2026-08-25T00:00:00Z') }], rowCount: 1 } as never; }); const app = Fastify(); registerTelemetryRoutes(app, verifier); await app.ready();
    ...
    expect(health.statusCode).toBe(200); expect(health.json().version).toBe('1.0.01'); expect(queues.json().data).toEqual({ pendingPayments: 2, pendingFieldCollections: 1, varianceBatches: 3 }); expect(audit.json().data[0].metadata).toEqual({ token: '[REDACTED]', national_id: '[REDACTED]', safe: 'ok' }); expect(query).toHaveBeenCalled(); await app.close();
  });

  it('masks nested sensitive telemetry fields without mutating safe values', () => { expect(maskTelemetry({ secret: 'x', nested: { borrower_id: 'b', value: 4 } })).toEqual({ secret: '[REDACTED]', nested: { borrower_id: '[REDACTED]', value: 4 } }); });
});
